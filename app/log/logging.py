...
def create_log_add_mailing(code_response: str, date_response: str, message_id: str):
    log_str = date_response + " || " + "Created mailing" + " || " + "Mailing_response.from_bytes" + " || " + "succsesful creating" + " || " + code_response + " || " + message_id + "\n"
    add_log(log_str)

def create_log_error_response(code_response: str, date_response: str, error_server: str = None):
    log_str = date_response + " || " + "Created mailing" + " || " + "Mailing_response.from_bytes" + " || " + f"server response error. {error_server}" + " || " + code_response + "\n"
    add_log(log_str)

def create_error_log(body: dict, status_response, code_response,date_response):
